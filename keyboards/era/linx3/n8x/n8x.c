...
bool process_record_kb(uint16_t keycode, keyrecord_t *record) {
    if (record->event.pressed) {
        switch (keycode) {
            case RGB_MOD_NEXT:
                g_my_config.backlight_effect++;
                if (g_my_config.backlight_effect > 3)
                    g_my_config.backlight_effect = 0;
                custom_config_set_value((uint8_t[]){id_custom_backlight_effect, g_my_config.backlight_effect});
                write_my_config_to_eeprom(&g_my_config);
                return false;
        }
